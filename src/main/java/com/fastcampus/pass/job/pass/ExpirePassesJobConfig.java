...
public class ExpirePassesJobConfig {
    ...
    public Step expirePassesStep() {
        return this.stepBuilderFactory.get("expirePassesStep")
                .<PassEntity, PassEntity>chunk(CHUNK_SIZE)// 청크 정의
     ...
     * 페이징 기법보다 높은 성능으로, 데이터 변경에 무결성 조회가 가능
     */
    @Bean
    @StepScope
    public JpaCursorItemReader<PassEntity> expirePassesItemReader() {
        return new JpaCursorItemReaderBuilder<PassEntity>()
                .name("expirePassesItemReader") // 아이템 리터 객체 이름 선언 및 생성
                .entityManagerFactory(entityManagerFactory) // jpa 로 데이터를 사용할 시 entityManagerFactory를 주입 해줘야함
                .queryString("select p from PassEntity p where p.status = :status and p.endedAt <= :endedAt") // 아이템 리더로 가져올 쿼리 선언
